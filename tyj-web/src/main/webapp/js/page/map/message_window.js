    ...
    $.each(data, function (i, o) {
        ...
        convertor.translate(pointArr, 1, 5, function (data) {
            if (data.status === 0) {
                ...
                var label = new BMap.Label(o.msgTypeName, opts);  // 创建文本标注对象
                label.setStyle({
                    color : "red",
                    fontSize : "12px",
                    height : "20px",
...
function doSearch() {
    ...
    var msgType = parent.$('#msgType').combobox('getValue');
    ...
    $.ajax({
        type: "post",
        async: false,
        url: "getMessageWindow",
        data: {deviceId: deviceId, startDate: startDate, endDate: endDate, msgType: msgType},
        dataType: "json",
        success: function (result) {
            if (success(result)) {
                ...
                for (var i = 0; i < ds.length; i++) {
                    var markers = [];
                    each(result.data[ds[i]], markers);
                    markerAll[ds[i]] = markers;
                }
            }
        },
